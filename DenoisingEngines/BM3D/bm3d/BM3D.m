       ...
       remains_PSD = process_psd_for_nf(remains_PSD, psd_k, profile);
        ...
        if(min(max(max(remains_PSD, [], 1), [], 2)) > 1e-5)
            ...
            [y_hat, ht_bm] = bm3d_thr_colored_noise(double(y_hat + remains), ...
                ...
                profile.Nstep, profile.N1, profile.N2, profile.lambda_thr3D2, ...
                profile.tau_match*profile.N1*profile.N1/(255*255), (profile.Ns-1)/2, ...
                single(t_forward), single(t_inverse)', ...
                [numel(profile.lambda_thr3D2); profile.lambda_thr3D2(:)], Wwin2D, ...
...
if bitand(stage_arg, BM3DProfile.WIENER_FILTERING)
    ...
    lambda_wiener = reshape(profile.lambda_wiener(:), [1, 1, numel(profile.lambda_wiener)]);
    lambda_wiener2 = reshape(profile.lambda_wiener2(:), [1, 1, numel(profile.lambda_wiener2)]);
    if single_dim_psd
    	lambda_wiener = sqrt(lambda_wiener);
    end
    ...
    [y_est, wie_bm] = bm3d_wiener_colored_noise(z, single(y_hat), hadper_trans_single_den, ...
        ...
        single(t_inverse)', Wwin2D, single(psd_blur.*lambda_wiener), profile.Nf, profile.Kin, ...
        blockmatches_wie);
    ...
    if (profile.denoise_residual)
        ...
        if(min(max(max(remains_PSD, [], 1), [], 2)) > 1e-5) 
            ...
            [y_est, wie_bm] = bm3d_wiener_colored_noise(double(y_est + remains), y_est, ...
                ...
                single(remains_PSD.*lambda_wiener2), profile.Nf, profile.Kin, blockmatches_wie);
...
function [lambda, wielambda, lambda2, wielambda2] = estimate_psd_parameters(PSD65_full)
...
for psd_num = 1:size(PSD65_full, 3)
    ...
    for ix = [1, 3]
        ...
        w1 = wielambdas(floor(param_idx2));
        w2 = wielambdas(min(ceil(param_idx2), numel(wielambdas)));
        ...
        if ix == 1
            ...
            wielambda = [wielambda, w2 * param_smooth2 + w1 * (1 - param_smooth2)];
        elseif ix == 3
            lambda2 = [lambda2, l2 * param_smooth + l1 * (1 - param_smooth)];
            wielambda2 = [wielambda2, w2 * param_smooth2 + w1 * (1 - param_smooth2)];
        end
...
auto_params = profile.lambda_thr3D == profile.NO_VALUE || ...
              profile.lambda_wiener == profile.NO_VALUE || ...
              (profile.denoise_residual && ...
                (profile.lambda_thr3D2 == profile.NO_VALUE || ...
                profile.lambda_wiener2 == profile.NO_VALUE) ...
              );
...
if auto_params && ~single_dim_psd
    ...
    [lambda_thr3D, lambda_wiener, lambda_thr3D2, lambda_wiener2] = estimate_psd_parameters(PSD65);
...
if profile.lambda_wiener == profile.NO_VALUE; profile.lambda_wiener = lambda_wiener; end
if profile.lambda_thr3D2 == profile.NO_VALUE; profile.lambda_thr3D2 = lambda_thr3D2; end
if profile.lambda_wiener2 == profile.NO_VALUE; profile.lambda_wiener2 = lambda_wiener2; end
