function [y_est] = BM3DDEB(z, sigma_psd, hhat , profile)
...
That = my_Fourier_filters(1,1,size(z,1),size(z,2),1);
That = That/max(That(:));
...
Regularization_alpha_RI = 4e-2;
Regularization_alpha_RWI = 5e-1;
...
% V = fft2(big_v); 
RI = conj(hhat)./( (abs(hhat).^2) + Regularization_alpha_RI .* sigma_psd .* That); % Transfer Matrix for RI    %% Standard Tikhonov Regularization
...
RWI  = conj(hhat).*Wiener_Pilot.^2./(Wiener_Pilot.^2.*(abs(hhat).^2) + Regularization_alpha_RWI .* sigma_psd .* That);   % Transfer Matrix for RWI (uses standard regularization 'a-la-Tikhonov')
zRWI = real(ifft2(fft2(z).*RWI));   % RWI OBSERVATION
PSD_zRWI = sigma_psd .* abs(RWI).^2;

y_est = BM3D(zRWI, PSD_zRWI, profile, y_hat);
y_est2 = BM3D(zRWI,sigma_psd,profile,y_hat);

figure(453);
subplot(2,3,1);imagesc(abs(fftshift(RI)));
subplot(2,3,4);imagesc(abs(fftshift(RWI)));
subplot(2,3,2);imagesc(zRI);
subplot(2,3,5);imagesc(zRWI);
subplot(2,3,3);imagesc(y_hat);
subplot(2,3,6);imagesc(y_est);
colormap(gray);
end