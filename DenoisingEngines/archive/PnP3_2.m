...
V = R(U,opts.sigma0/sqrt(beta)); % splitting variable, V
gA = mu*(A(A(U(:),1),2)-Atb);
...
while ii < opts.iter % iterations refers to outer loops
    ...
    figure(345);
    subplot(2,2,1);imagesc(U);title('u');colorbar;
    subplot(2,2,2);imagesc(V);title('v');colorbar;
    subplot(2,2,3);imagesc(sigma);title('sigma');colorbar;
