...
out.TVnorm = computeHOTVnorm(u,lam);
for i = 1:opts.iter
   bDHu = (b+epsilon)./(Hu+epsilon); % b/h*u
   up = u;
   TVterm = myHOTVEulerLagrange(u,1e-1)*lam;
   % algorithm is slightly more stable by splitting TVterm into positive
   % and negative parts
   S1 = double(TVterm>=0);  S2 = abs(S1-1);
   u = max(u.*(ifftn(conj(hhat).*fftn(bDHu))-TVterm.*S2)./(1+TVterm.*S1),1e-3);
   ...
   out.TVnorm = [out.TVnorm;computeHOTVnorm(u,lam)];
end
out.iters = i;
out.logLikelihood = out.objF - sum(b(:).*log(b(:))) + sum(b(:));

function uTV = computeHOTVnorm(u,lambda)

ux = diff(u,2,2);
uy = diff(u,2,1);
uTV = lambda*sum(sum(sqrt(ux(1:end-2,:).^2 + uy(:,1:end-2).^2)));
...
sumHg = sum(Hg(:));
sumHu = sum(Hu(:));      
objFold = -sum(Hu(:)) + sum(b(:).*log(Hu(:))) - computeHOTVnorm(up,lam);
objFnew = -(sumHu+sumHg) + sum(b(:).*log(Hu(:) + Hg(:))) - computeHOTVnorm(up+g,lam);
tau = 1;
