...
u = ifft2(sum(fft2(b).*hhat,3))/nF; % initial guess for u
up = u;
...
for i = 1:opts.iter
   ...
   Hy = ifft2(hhat.*fft2(y)); % h*y
   bDHy = (b+epsilon)./(Hy+epsilon); % ratio
   if opts.lambda == 0
       % u(x) <- u(x)*(h(-x)*(b/h*u))
