...
hhat = zeros(size(b,1),size(b,2));
hhat([1:2,end],[1:2,end]) = 1/9;
hhat = fft2(hhat); % 9 pixel averaging kernel
...
for i = 1:opts.iter
   ...
   if ~superRes
   out.objF = [out.objF;-sum(Hy(:)) + sum(b(:).*log(Hy(:)))-lam*sum(normR(:))];
   else
