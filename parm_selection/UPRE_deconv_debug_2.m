function [U,out] = UPRE_deconv(G,K,b,opts,sigma)
...
for i = 1:maxiter
    ...
    if isfield(opts,'Utrue')
        out.eetrue = [out.eetrue;norm(col(ifftn(Khat.*fftn(u-opts.Utrue))))^2];
    end    
    
    % new mu, updated based on the fixed point
    % (u'T'TH^{-1}T'Tu)/(sigma^2*trace(AH^{-1}T'TH^{-1}A'))
    tmp = V.*Fu; tmp2 = tmp./(1+V/mu);
    mu = sum(col(real(ifft(tmp)'*ifft(tmp2))))/(sigma^2*trBIG);
