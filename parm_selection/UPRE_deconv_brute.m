function [U,out] = UPRE_deconv(K,b,opts,sigma,mumin,mumax,Nmu)
...
Fsig = false;
if nargin<4, error('must specify sigma');
elseif nargin<5, mumin = 1e-5;mumax = 1e5;Nmu = 20;
elseif nargin<6, mumax = 1e5; Nmu = 20;
elseif nargin<7, Nmu = 20;
end
...
for i = 1:Nmu
    ...
    trHiAA = sum(Khat2(:)./(V(:)/mus(i) + Khat2(:)));
    ...
    % compute UPRE
    out.UPREs(i) = -m*sigma^2 + (re'*re) + 2*sigma^2*trHiAA;
    if out.UPREs(i)<out.UPREmin
        out.UPREmin = out.UPREs(i);
        FuBest = Fu;
    end
