...
if size(b,3)~=size(h,3), error('dimension mismatch'); end

x = mean(b,3);
xp = x;
hhat = fft2(h);
hhat2 = hhat.*conj(hhat);
Atb = ifft2(sum(conj(hhat).*fft2(b),3));
out.rel_chg = [];
mu = opts.mu;
out.objF = [];
Fx = fftn(x);
tau = 1/max(abs(hhat2(:)))/size(b,3);
for i = 1:opts.iter
    ...
    xp = x;
    x = y - real(tau*(ifft2(sum(hhat2.*fft2(y),3))-Atb));
