function [U, out] = HOTV3D_PBA(A,b,n,opts)
...
% reshape stack to sinogram to column vector
...
foversamp = 30; out.shifts_all = [];
if ~isfield(opts,'PBA_updates'), opts.PBA_updates = 4; end
if ~isfield(opts,'kmax'), opts.kmax = 2; end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while numel(out.f) <= opts.max_iter
    ...
    % end of inner loop
     ...
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     %%%%%%%%%%%%%% PBA  section %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     if ii<opts.PBA_updates+1
        [sino,shifts] = tomo_PBA...
            (Wradon,reshape(U,p,q,r),sino,foversamp,opts.kmax);
