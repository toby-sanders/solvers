function [U, out] = shearL1(A,b,n,opts)
...
%
% where D^k is order k Daubechies wavelet map.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%  Changes for the shearlets
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global D Dt
if q==1, error('shearlets should be used for 2D or 3D signals');
else % [D,Dt] = my_wavelet_trans_2D(['db',num2str(k)],opts.levels,p,q);
shear_sys = SLgetShearletSystem2D(0,p,q,4);
D = @(X)shearlet_transform(X,shear_sys,1);
Dt = @(X)shearlet_transform(X,shear_sys,2);
end
