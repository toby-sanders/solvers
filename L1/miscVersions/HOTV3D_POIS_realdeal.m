function [U, out] = HOTV3D_POIS_realdeal(A,b,n,opts)
...
%
%      min_{u,w} {mu*E(u) + beta/2 ||D^k u - w ||_2^2 
%               + ||w||_1  - (sigma , D^k u - w) }
%
...
[lam2,lam3,lam4,lam5,f,gD,Au,gA] = get_grad(U,Uc,W,...
    lam1,beta,mu,A,b,At1,sigma,delta);
...
while numel(out.f) <= opts.max_iter
    ...
    if mod(ii,2)==0     
        ...
        [mu,flg_inc,flg_dec,out] ...
            = get_mu(A(U(:),1),b,mu,stepmu,max_mu,min_mu,flg_dec,flg_inc,out);
        muDbeta = mu/beta;
    ...
    f = lam1 + beta/2*lam2 + mu*lam3 - lam4 - lam5;
