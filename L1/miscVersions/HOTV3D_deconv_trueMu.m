function [U, out] = HOTV3D_deconv(K,b,n,opts,Utrue)
...
%
% Fast signal, image, and video deconvolution with HOTV
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         Problem Description       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while iter <= opts.iter
    ...
    for jj = 1:opts.inner_iter
        iter = iter + 1;
        Up = U;
        bb = fftn(mu*Atb+reshape(Dt(beta*W + sigma),p,q,r));
        U = ifftn(bb./(mu*Ghat2 + beta*V));
...
out.final_error = norm(ifftn(Ghat.*fftn(U))-b)/norm(b(:));
