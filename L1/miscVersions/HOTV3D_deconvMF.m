function [U, out] = HOTV3D_deconvMF(K,b,opts)


% Written by Toby Sanders @Lickenbrock Tech 
%
% Fast multiframe deconvolution with HOTV
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         Problem Description       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% K is a series of 2D blurring kernel (input as a 3D matrix)
...
%
% Inputs: 
%   K: 3D matrix of 2D blurring kernels
%   b: 3D matrix of blurred images
...
tic;
[p,q,nF] = size(K); r = 1;
% if size(b)~=[p,q,nF], error('b should match PSF size'); end
...
if ~wrap_shrink, ind = get_ind(k,p,q,1);
else, ind=[]; end
[D,Dt] = get_D_Dt(k,p,q,1,opts,col(b));
flg = check_D_Dt(D,Dt,[p,q,1]);
if ~flg, error('Sparse domain transforms do not appear consistent'); end
U = zeros(p,q); 
sigma = D(U);  % initialize multipler
W = sigma;
% nrmb = norm(b);
V = get_myFilters(k,nlev,p,q,1);

out.rel_chg = [];
ii = 0; iter = 0;
Ghat = fft2(K);
Ghat2 = sum(Ghat.*conj(Ghat),3);
Atb = sum(ifft2(fft2(b).*conj(Ghat)),3);
while iter <= opts.iter
    ii = ii + 1;    
    for jj = 1:opts.inner_iter
        iter = iter + 1;
        Up = U;
        bb = fft2(mu*Atb+reshape(Dt(beta*W + sigma),p,q,1));
        U = ifft2(bb./(mu*Ghat2 + beta*V));
        if opts.nonneg, U = max(real(U),0);
        elseif opts.isreal, U = real(U); end
        if opts.max_c, U = min(U,opts.max_v); end        
        U = reshape(U,p,q);  Uc = D(U);
...
% out.final_error = norm(ifft2(Ghat.*fft2(U))-b)/nrmb;
