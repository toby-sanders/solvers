function [D,Dt] = FD2D_multiscale(k,levels,p,q,PSD)
...
for ii = 1:levels
    ...
    [~,mm] = max(vy(:));
    lambdas(ii) = PSD(mm);% lambda = max(lambda,1);
end
lambdas = max(lambdas/max(lambdas),.25);

for ii = 1:levels
    vx = (-1)^k*[0,1/l(ii)*((exp(-1i*2*pi*(1:q-1)*l(ii)/q)-1).^(k+1))./(exp(-1i*2*pi*(1:q-1)/q)-1)];
    vy = (-1)^k*[0,1/l(ii)*((exp(-1i*2*pi*(1:p-1)*l(ii)/p)-1).^(k+1))./(exp(-1i*2*pi*(1:p-1)/p)-1)];
    [VX(:,:,ii),VY(:,:,ii)] = meshgrid(vx*lambdas(ii),vy*lambdas(ii));
end
