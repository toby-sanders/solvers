function [U, out] = HOTV3D(A,b,n,ghat,opts)




% ghat is the power spectrum of the noise
% this function only really works for deblur
% the test file is in the "coloredNoiseWiener" folder in my NOTES files
...
elseif strcmp(opts.mode,'deconv')
    ...
    params.Atb = ifftn(fftn(b).*conj(A)./ghat2);
    A = A.*conj(A); 
...
while numel(out.rel_chg_inn) < opts.iter
    ...
    for jj = 1:opts.inner_iter
        ...
        [U,params] = updateU_HOTV_colored(A,D,Dt,U,Uc,W,gL,mu,beta,params,opts,ghat2);
        Uc = D(U);
...
function [U,params] = updateU_HOTV_colored(A,D,Dt,U,Uc,W,gL,mu,beta,params,opts,ghat2)
...
switch params.mode % handling various update types
    ...
    case 'deconv' % updates for deconvolution 
        ...
        U = ifftn(bb./(mu*A./ghat2 + beta*params.V));
end
