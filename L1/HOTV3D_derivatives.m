...
elseif strcmp(opts.mode,'deconv')
    ...
    [D1,D1t] = get_D_Dt(1,p,q,r,opts);
    [D2,D2t] = get_D_Dt(2,p,q,r,opts);
    D1b = D1(b); 
    D1b = reshape(D1b,p,q,r,3);
    DtDb1= D1t(D1b);
    D2b = D2(b); 
    D2b = reshape(D2b,p,q,r,3);
    DtDb2= D2t(D2b);
    params.Atb = ifftn(fftn(b+Dscl*reshape(DtDb1,p,q,r) ++Dscl*reshape(DtDb2,p,q,r)).*conj(A));
    % params.Atb = params.Atb + reshape(DtDb,p,q,r);
   %  b = [b;Dscl*D1b(:,:,1,1);Dscl*D1b(:,:,1,2)];   
    A = A.*conj(A);
    V1 = my_Fourier_filters(1,1,p,q,r);
    V2 = my_Fourier_filters(2,1,p,q,r);
    A1 = Dscl*conj(A).*V1.*A;
    A2 = Dscl*conj(A).*V2.*A;
    A = A + A1 + A2;
