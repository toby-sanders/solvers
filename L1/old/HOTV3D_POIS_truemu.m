function [U, out] = HOTV3D_POIS_truemu(A,b,n,opts,Utrue)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This part for Poisson noise
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
epsilon = .1;
if sum(b<0)
    error('data vector b should be nonnegative');
end
...
out.DU = [out.DU;norm(Uc(:),1)];out.err_true =1;
...
for ii = 1:opts.outer_iter
    ...
    btmp = AO(U(:),1);    % new estimate for b
    [btmp,~] = Scaleb(btmp);
    R = 1./(sqrt(btmp)+epsilon); % weights
    R = R/max(R);   % rescale them
    % new weighted operator and weighted b and A'*b
    A = @(u,mode)my_local_oper(AO,R,u,mode)/mscl;
    b = borig.*R*scl/mscl;
    Atb = A(b,2);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
