...
for ii = 1:opts.outer_iter
    ...
    if ~opts.smooth_phase
        if opts.update_phase && mod(ii,2)==1
            if round(k) == k && opts.levels == 1
                [D,Dt] = FD3D_complex(k,p,q,r,angle(U)); % standard complex FD
            elseif opts.levels == 1
                [D,Dt] = FFD3D_complex(k,p,q,r,angle(U)); % fractional complex FD
            else
                [D,Dt] = FD3D_multiscale_complex(k,opts.levels,p,q,r,angle(U));  % multiscale complex FD 
            end
            phase_flg = true;
