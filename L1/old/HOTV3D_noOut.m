function [U, out] = HOTV3D_noOut(A,b,n,opts)

% a "no output" version of the code, which also does not use backtracking.
...
% first shrinkage step and gradients
[W,gD] = shrinkage(Uc,L1type,beta,sigma,wrap_shrink,ind);
Au = A(U(:),1);
gA = A(Au,2) - Atb;
...
ii = 0;  % main loop
itercnt = 0;
while itercnt < opts.iter
    ii = ii + 1;
    % Steepest Descent at the 1st iteration
    ...
    for jj = 1:opts.inner_iter
        ...
        if (rel_chg_inn < tol_inn) || itercnt>=opts.iter, break; end 
    end
    ...
    % update multipliers
    deltap = delta;
    [sigma,delta] = update_mlp(beta,mu,W,Uc,Au,b,sigma,delta);
    if ii>=opts.data_mlp, delta = deltap; end% lam5 = lam5p;  end
...
out.total_iter = itercnt;
out.elapsed_time = toc;


function [sigma,delta] = update_mlp(beta,mu, ...
...
function [W,gD] = shrinkage(Uc,L1type,...
    beta,sigma,wrap_shrink,ind)
