function [U, out] = HOTV3D_AF(A,b,n,opts)
...
[nray,nslice,nangles] = size(b);
b = stack2sino(b);
b = reshape(b,nray*nangles,nslice);
...
p = n(1); q = n(2); r = n(3);
Wradon = A;
A = @(x,mode)mm_operator(x,mode,Wradon,nslice,[p,q]);
...
%b = reshape(b,nray*nangles,nslice);
sino = reshape(b,nray,nangles,nslice);


% autofocusing weapons
[nray,nangles,nslice] = size(sino);
foversamp = 30;
if ~isfield(opts,'AF_updates')
    opts.AF_updates = 4;
end
%N = foversamp*nray; % N: length of fft
%freqs = linspace(0,2*pi*(1-1/N),N);
%freqs = repmat(freqs',1,nangles,nslice);
...
for ii = 1:opts.outer_iter
    ...
    % let's find some shifts
    % compute N-point fourier transforms
    %{
    sinof = fft(sino,N);
    sinor = reshape(A(U(:),1),nray,nangles);
    sinorf = fft(sinor,N);
    % compute phase differences and apply shifts
    fshift = imag(log(sinorf./sinof))./freqs;
    shifts = -round(mean(fshift(2:foversamp,:)));
    % now shift the sinogram
    for iii = 1:nangles
        sino(:,iii) = circshift(sino(:,iii),[shifts(iii),0]);        
    end
    %}
    if ii<opts.AF_updates+1
        [sino,shifts] = tomo_autofocus...
            (Wradon,reshape(U,p,q,r),sino,foversamp,2);
        out.shifts_all = [out.shifts_all;shifts];
        % reset b and recalculate A'*b
        b = sino(:);
        Atb = A(b,2);
    end
...
elseif strcmp(L1type,'isotropic')
    Ucbar = Uc - sigma/beta;
    Ucbar_norm = sqrt(Ucbar(:,1).*conj(Ucbar(:,1)) + ...
        Ucbar(:,2).*conj(Ucbar(:,2)) + Ucbar(:,3).*conj(Ucbar(:,3)));
    Wbar = max(Ucbar_norm - 1/beta, 0)./(Ucbar_norm+eps);
    W(:,1) = Ucbar(:,1).*Wbar;
    W(:,2) = Ucbar(:,2).*Wbar;
    W(:,3) = Ucbar(:,3).*Wbar;
    ...
    error('L1type is either isotropic or anisotropic');
end
