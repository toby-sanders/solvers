...
opts.mu0 = opts.mu/4;  % works well for this algorithm
...
A = @(u,mode)l1_l1_operator(A,D,Dt,u,mode,numb,opts.mu0);
tmp = D(zeros(p*q*r,1));
b = opts.mu0*[b;zeros(numel(tmp),1)];
clear tmp;
...
[U,beta,betaf,mu,muf,delta,gL,out] ...
    = get_l1solver(p,q,r,Atb,A,scl,opts,b);    % U: p*q
    ...
    for jj = 1:opts.inner_iter
        ...
        %if jj~=1
            ...
            if jj~=1
                ...
                Ag = A(g,1);
                tau = ((g'*g)/(beta*(Ag')*Ag));
                %tau = abs((beta*(Ag'*(Aub-W))-delta'*Ag)/(beta*Ag'*Ag));
            end
...
%                 [U,lam1,lam3,f,Au,gA] = back_up(lam2,...
...
out.final_error = sum(abs(A(U(:),1)-b))/nrmb;
...
function [U,lam1,lam3,f,Au,gA] = back_up(lam2,...
    alpha,beta,Up,du,gAp,dgA,Aup,dAu,W,b,delta)

gA = gAp + alpha*dgA;
U = Up + alpha*du;%reshape(du,p,q,r);
Au = Aup + alpha*dAu;
Aubw = Au-b-W;
