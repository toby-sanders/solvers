...
L1type = opts.L1type;
% indices aren't set up for these cases,�\_(?)_/� 
% so just do shrinkage everywhere
...
out.DU = [out.DU;norm(Uc(:),1)];out.g = g'*g;
for ii = 1:opts.outer_iter
    if opts.disp
            fprintf('    Beginning outer iteration #%d\n',ii);
            fprintf...
                ('iter    ||w||_1    ||Du - w||^2  ||Au - b||^2   rel chg\n');
    end
...
Aub = Au-b;
if ii==1
    y = mu/beta*A(Aub,2)+Dt(V);
    M = @(x,mode)myhandle(Dt,D,x,mode);
    sigma = run_cgs(M,y,1e-12,5);
    sigma = reshape(sigma,size(V,1),size(V,2),size(V,3),size(V,4));
else
