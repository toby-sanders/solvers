function [U, out] = PA3D_hard(A,b,p,q,r,opts)
%Try the code with hard thresholding
...
for ii = 1:opts.maxit
    ...
    while f > C - alpha*const
        if cnt == 5
            ...
            Wx = Uxbar.*(abs( Uxbar > 1/beta));
            Wy = Uxbar.*(abs( Uybar > 1/beta));
            Wz = Uxbar.*(abs( Uzbar > 1/beta));
            %reset edge values if not using wrap around regularization
            if ~wrap_shrink
                if ko<=q, Wx(:,end-ko+1:end,:) = Ux(:,end-ko+1:end,:); end;
                if ko<=p, Wy(end-ko+1:end,:,:) = Uy(end-ko+1:end,:,:); end;
    ...
    if (RelChg < tol_inn || ii-sum_itrs >= opts.maxin)% && ~fst_itr
        ...
        if RelChgOut < tol_out || count > opts.maxcnt || sqrt(lam3(end))/nrmb<opts.min_l2_error
