function [U, out] = HOTV3D_RW(A,b,n,opts)
...
global D Dt
lp = 0.5;
[D,Dt] = my_local_D_Dt(k,p,q,r);
[U,mu,beta,muf,betaf,muDbeta,sigma,delta,gL,ind,out] ...
    = get_PA3D_RW(p,q,r,Atb,scl,opts,k,b,wrap_shrink);    % U: p*q
...
[lam2,lam3,lam4,lam5,f,gD,Au,gA] = get_grad(U,Uc,W,...
    lam1,beta,mu,A,b,Atb,sigma,delta,w);
...
for ii = 1:opts.outer_iter
    ...
    gL = 1/beta*(Dt(sigma,w) + A(delta,2));
...
function [D,Dt] = my_local_D_Dt(k,p,q,r)
...
D = @(U,c)D_Forward(U,k,p,q,r,c);
Dt = @(Uc,c)D_Adjoint(Uc,k,p,q,r,c);
