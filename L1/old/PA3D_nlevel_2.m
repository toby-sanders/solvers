function [f, out] = PA3D_nlevel(A,b,p,q,r,opts)
...
for ii = 1:opts.outer_iter
    ...
    for jj = 1:opts.inner_iter
        ...
        if lam3 > epsb
            ...
            gf = mu*gA + beta*gD - gsigma - gdelta; %gradient of f subproblem
            ...
            if jj==1
                ...
                dgD = gD - gDp;
                s1 = ffp'*ffp;
                s2 = ffp'*(beta*dgD + mu*dgA);
                tauf = -s1/s2;
            end

            fp = f; gAp = gA; gDp = gD;  % keep previous values
    ...
    [sigma,delta,~,~] = update_mlp(beta,mu,...
        w,fc,Afb,sigma,delta);
   
    
end

out.total_iter = jj*ii;
out.final_error = norm(Af-b)/nrmb;
