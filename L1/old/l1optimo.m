...
opts = check_l1_opts(opts);  % get and check opts
...
opts.mu0 = opts.mu;
[U,mu,beta,muDbeta,sigma,delta,gL,out] ...
    = get_l1optimo(p,q,r,Atb,scl,opts,b,D); 
nrmb = norm(b);
Uc = D(U);

% first shrinkage step
[W,lam1] = shrinkage(Uc,beta,sigma,[],[],[],[],0);
...
while numel(out.f) <= opts.iter
    ...
    for jj = 1:opts.inner_iter
        ...
        if alpha ~= 0
            [W,lam1,lam2,lam4,f,gD] = shrinkage(Uc,...
                beta,sigma,lam1,lam2,lam4,f,1);
        end
...
function [W,lam1,lam2,lam4,f,gD] = shrinkage(Uc,...
    beta,sigma,lam1,lam2,lam4,f,flg)
