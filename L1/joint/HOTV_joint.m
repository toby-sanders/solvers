function [U, out] = HOTV_joint(A,b,n,opts)
...
if ~isfield(opts,'zeta')
    warning('zeta not specified, using default');
    opts.zeta = 50;
end
...
if opts.smooth_phase
    [DZ,DZt] = FDZ(opts.orderz,p,q,r);
else
    [DZ,DZt] = FDZ_complex(opts.orderz,p,q,r,exp(-1i*opts.phase_angles));
end
...
A = @(u,mode)l2_operator(A,DZ,DZt,u,mode,numel(b),sqrt(opts.zeta/opts.mu));
tmp = DZ(zeros(p*q*r,1));
num_b_or = numel(b);
b = [b;zeros(numel(tmp),1)];
clear tmp;
...
[U,mu,beta,muf,betaf,muDbeta,sigma,delta,gL,ind,out] ...
    = get_HOTV_joint(p,q,r,Atb,scl,opts,k,b,wrap_shrink);    % U: p*q
...
Aub = A(U(:),1)-b;
out.final_error = norm(Aub(1:num_b_or))/nrmb;%norm(A(U(:),1)-b)/nrmb;
out.final_Duz = norm(Aub(num_b_or+1:end));
...
function y = l2_operator(A,DZ,DZt,u,mode,d1,zeta)
