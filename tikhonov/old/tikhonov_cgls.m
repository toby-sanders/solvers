...
%
%  X = argmin_x  ||Ax - b||_2^2 + lambda^2 ||Dx||_2^2
%
...
if opts.order ~= 0
    bpad = [b;zeros(opts.levels*3*dim1*dim2*dim3,1)];
else
    bpad = [b;zeros(dim1*dim2*dim3,1)];
end

%define the finite difference operators
k = opts.order;
if round(k) == k & opts.levels == 1
        [D,Dt] = FD3D(k,dim1,dim2,dim3);  % standard FD
elseif opts.levels == 1
        [D,Dt] = FFD3D(k,dim1,dim2,dim3); % fractional FD
elseif round(k)==k
        [D,Dt] = FFD3D_multiscale(k,opts.levels,dim1,dim2,dim3);  % multiscale FD
else
        [D,Dt] = FD3D_multiscale(k,opts.levels,dim1,dim2,dim3); % multiscale fractional FD
end
...
A = @(u,mode)tikhonov_operator_local(Amain,D,Dt,...
    dim1,dim2,dim3,datadim,u,mode,opts.lambda,opts.order,opts.levels);





function y = tikhonov_operator_local(A,D,Dt,dim1,dim2,dim3,datadim,u,mode,lambda,order,levels)
